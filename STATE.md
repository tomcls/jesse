...
- 23:4x — period sweep BTC 1h terminé: p48=0.937 p56=0.806 p64=1.1045 p80=0.811(DD-34.6 ❌). Colline pointue → PERIOD FIGÉ À 64. NEXT: grille long/short mult autour de (4.8,4.1) sur p64: essayer (4.2,4.1) (5.4,4.1) (4.8,3.6) (4.8,4.6) (5.2,4.4). Puis vol gate 1.1/1.35, atr_mult 1.4, risk 1.4. Puis TF 2h (period ~32-40). Puis SOL.
- 00:0x — grille mults BTC 1h p64: (4.8,4.1)=1.1045 | (4.2,4.1)=1.1679/DD-18.1/+28.0 | (3.8,4.1)=1.0966 | (4.2,4.6)=1.2526/DD-19.8/+30.0 PF1.60 ★ | (4.2,5.1)=voir ALL-RUNS. Long calé 4.2, short: plus large = mieux (shorts sélectifs). NEXT selon (4.2,5.1): si >1.2526 → (4.2,5.6); sinon short calé 4.6-5.1 → vol gate 1.1/1.35, atr_mult 1.4-1.5, atr_sma_period 24, risk 1.4. Puis TF 2h. Puis SOL (R4_SOLKeltnerAsym à créer: partir 2h period 40, mults larges (5,5), vol gate 1.2).
- 00:2x — mults BTC 1h p64 suite: (4.2,5.6)=1.3530/DD-19.8/+31.6/PF1.73/160t ★★ | (4.2,6.2)=voir ALL-RUNS dernière ligne. Si (4.2,6.2)>1.353 → tester long-only pur (shorts off). Sinon short calé ~5.6. Ensuite: vol gate 1.1/1.35, atr_sma_period 24, atr_mult 1.4, risk 1.4-1.5. Puis TF 2h (p32). Puis SOL.
- 00:4x — ★★★ JALON: BTC 1h p64 (4.2,6.8) = Sharpe 1.5319, DD -15.52%, +35.9%/an, PF 2.01, 136t. OBJECTIF SHARPE 1.5 ATTEINT, critères durs OK. Progression mults: 4.1→1.10, 4.6→1.25, 5.1→1.29, 5.6→1.35, 6.2→1.48, 6.8→1.53. NEXT: (4.2,7.4), long-only pur, puis ROBUSTESSE voisinage (period 60/68, long 4.0/4.4, short 6.4/7.2, vol gate, atr) — indispensable vu la pente à sens unique (risque: shorts quasi-off = le vrai signal est peut-être long-only). Puis SOL.
- 01:0x — (4.2,7.4)=1.3959/DD-24.5 → sommet = (4.2,6.8). Les shorts rares-mais-présents protègent le DD (-15.5 vs -24.5). Voisinage en cours: (4.4,6.8)=voir ALL-RUNS. RESTE: (4.0,6.8), (p60,4.2,6.8), (p68,4.2,6.8), vol gate 1.2/1.3, atr_mult 1.4, risk 1.4. PUIS SOL EN PRIORITÉ (ne pas passer la nuit sur BTC — le candidat 1.5319 suffit, la robustesse peut se compléter au matin).
- 01:3x — BTC voisinage: (4.4,6.8)=1.5701/DD-14.5/+36.7 ★ MEILLEUR. SOL: config BTC sur 1h=0.3296/DD-26.7 (rejet). SOL 2h p40 (5.0,6.0) vol1.15 atr1.4 risk1.4 = voir ALL-RUNS. Pistes SOL suivantes: p56 2h, (5.5,7.0), vol gate 1.0 (off) vs 1.25, TF 4h p24, long-only SOL, ou fade-rallye short-only (SOL 2025 = -60% → short leg peut porter). BTC robustesse restante: (4.0,6.8), p60/p68, vol/atr/risk sweeps.
- 02:3x — SOL Keltner: 2h no-gate=0.132 ❌ (gate aide) | 2h p56=0.5425/68t/+8.1%an/DD-27.1 | 4h p24=voir ALL-RUNS. Si 4h <0.7 → PIVOT FAMILLE SOL: trend-follow 4h (EMA cross + trailing ATR, long-biased risk sizing) — cf proto spot où SMA200-régime marchait le mieux sur SOL. BTC candidat (4.4,6.8) tient toujours: 1.5701.
- 03:0x — SOL PIVOT trend-follow 4h EMA21/55 + trailing ATR: v1 (trail3.0,risk1.5)=0.5822/251t/+8.0%an/DD-14.1 (meilleur profil SOL, DD sain, churn élevé). v2 (trail4.5,atr3.5,risk2.0)=voir ALL-RUNS. Leviers restants: shorts OFF, ema 13/48 ou 26/80, risk 2.4, TF 6h. Rappel cible SOL: +20%/an, DD<=25, Sharpe secondaire.
- 03:4x — SOL TF: long-only risk2.4 = 0.6744/+12.5%an/DD-16.1 (shorts aident: garder ON). risk2.8 shorts ON = voir ALL-RUNS. Si DD<25 et annual proche 20 → affiner EMAs (13/48, 26/80) pour la qualité. Sinon risk 2.6 et EMAs plus lents.
- 04:1x — SOL TF EMAs: 26/80=0.5282/DD-29.4 ❌ | 13/48=voir ALL-RUNS. 21/55 reste la réf (risk2.8: 0.7251/+17.5%an/DD-24.0 — borderline; risk2.4+shorts: ~0.72/+13.4/DD-17.7). Options si 13/48 ne perce pas: trail sweep 4.0/5.0, atr_mult 3.0/4.0 à 21/55 risk 2.6; ou accepter le meilleur profil et documenter honnêtement l'écart vs cible +20%an.
- 04:3x — ★★★ JALON SOL: R4_SOLTrendFollow 4h EMA13/48, atr3.5, trail4.5, risk2.5, shorts ON = Sharpe 0.8441, +20.43%/an, DD -19.76%, 144t, PF 1.47. CRITÈRES DURS TOM SATISFAITS (annual>=20 ✓, DD<=25 ✓). BTC déjà OK: (4.4,6.8)=1.5701/+36.7%an/DD-14.5. RESTE (robustesse): voisins SOL (12/44, 14/52, trail 4.0/5.0, atr 3.2/3.8, risk 2.3), voisins BTC ((4.0,6.8), p60/p68, vol gate 1.2/1.3), corrélation R4-BTC vs R4-SOL vs R2-ETH (dedup), rapport final 07:50.
...
- R3 Jesse portefeuille: sizing portfolio_value seul = DD -39.2 ❌ (dérive des poches sans rebal). FIX: rééquilibrage à bandes 20% dans update_position via VRAIE API Jesse (trim = self.take_profit au prix courant → market reduce; add = self.buy en position). NOTE API: reduce_position/increase_position N'EXISTENT PAS (ignorés silencieusement — piège!). Résultat portefeuille 3 routes (6aee5f66): Sharpe 0.7343, +21.1%/an, DD -27.58%, 26 trades, PF 3.08 — conforme au proto Python (-28.5% attendu), objectif DD<=~-30 tenu, vs B&H +16%/-73%.
