...
class MeanRevZscore(Strategy):
    """
    Mean reversion: enter when the z-score of price vs its rolling mean
    exceeds a threshold, exit when z-score reverts to zero.
    """
    ...
    def update_position(self):
        if self.is_long and self.z >= 0:
            self.liquidate()
        if self.is_short and self.z <= 0:
            self.liquidate()

    def hyperparameters(self) -> list:
        return [
            {'name': 'period', 'type': int, 'min': 10, 'max': 40, 'default': 20},
            {'name': 'threshold', 'type': float, 'min': 1.0, 'max': 3.5, 'step': 0.1, 'default': 2.0},
